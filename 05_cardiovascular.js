...
(function () {
  ...
  // Evita conflictos de IDs cuando se renderizan múltiples gráficos en la misma página
  let ekgCounter = 0;

  // --- 1. HELPER DE TRADUCCIÓN (Internal) ---
  const t = (es, en) => `<span class="lang-es">${es}</span><span class="lang-en hidden-lang">${en}</span>`;

  // --- 2. GENERADOR EKG (SVG Vectorial) ---
  // Dibuja trazados médicos sin imágenes externas para máxima velocidad y nitidez.
  const getEKG = (type) => {
    // Colores dinámicos para modo claro/oscuro
    const strokeColor = "stroke-slate-900 dark:stroke-red-400";
    const gridColor = "text-red-100 dark:text-red-900/20";
    
    // ID único para cada patrón SVG (evita duplicados en DOM)
    const uniqueId = `grid-${type}-${ekgCounter++}-${Math.random().toString(36).substr(2, 9)}`;
    
    // Grid de fondo (Papel milimetrado médico)
    const grid = `
      <defs>
    ...
    let path = '';
    // Trazados vectoriales simplificados para fines educativos
    switch(type) {
      ...
      case 'block3': path = "M0,55 h10 c2,-6 5,-6 8,0 h15 l5,-35 l5,70 l5,-35 h15 c2,-6 5,-6 8,0 h25 c2,-6 5,-6 8,0 h10 l5,-35 l5,70 l5,-35 h10 c2,-6 5,-6 8,0"; break; // 3rd Degree Block (Lethal)
    ...
    return `
      <div class="w-full h-24 bg-white dark:bg-black/40 rounded-lg border border-red-100 dark:border-red-900/30 overflow-hidden relative my-3 shadow-inner group-hover:border-red-300 transition-colors" role="img" aria-label="EKG strip of ${type}">
  ...
  NCLEX.registerTopic({
    ...
    // 2. GUÍA RÁPIDA EKG (Con 3rd Degree Block - V6+)
    renderEKGStripGuide() {
      return `
        <section>
    ...
    // 3. RITMOS CRÍTICOS (Intervención)
    ...
    // 4. ALGORITMOS ACLS (AHA 2020: Atropina 1mg)
    ...
    // 5. VASCULAR COMPARISON (PAD vs PVD)
    ...
    // 7. PHARMACOLOGY (Safety Integrated from v5)
    renderPharmacology() {
      return `
        <section>
