...
// MEJORADO: Modo Estudio con persistencia, actualización de idioma y eventos delegados
...
(function() {
  ...
  // --- ESTADO LOCAL DEL MODO ESTUDIO ---
  const STORAGE_KEY = 'nclex_study_mode';
  let studyModeActive = localStorage.getItem(STORAGE_KEY) === 'true';
  ...
  // --- Función para actualizar el texto del botón según idioma y estado ---
  function updateToggleButton(btn, active) {
    ...
    // Aplicar visibilidad de idioma actual
    if (window.applyGlobalLanguage) window.applyGlobalLanguage();
  }
  ...
  function initInteractions() {
    ...
    // Delegación de eventos para revelar números al hacer clic
    const handleLabClick = (e) => {
      if (!content.classList.contains('study-mode')) return;
      const target = e.target.closest('.lab-value');
      if (target) {
    ...
    content.removeEventListener('click', handleLabClick);
    content.addEventListener('click', handleLabClick);

    // Configurar botón toggle
    const toggleMode = () => {
      studyModeActive = !studyModeActive;
      localStorage.setItem(STORAGE_KEY, studyModeActive);

      if (studyModeActive) {
        content.classList.add('study-mode');
        // Ocultar todos los números revelados
        content.querySelectorAll('.lab-value.revealed').forEach(el => el.classList.remove('revealed'));
      } else {
        content.classList.remove('study-mode');
      }

      updateToggleButton(toggleBtn, studyModeActive);
    };
    ...
    const handleLangChange = () => {
      if (toggleBtn) updateToggleButton(toggleBtn, studyModeActive);
    };
  ...
  NCLEX.registerTopic({
    ...
    render() {
      // Iniciar interacciones después de pintar el DOM
