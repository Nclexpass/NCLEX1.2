...
(function () {
  ...
  // --- SISTEMA DE SONIDO EKG (Beep realista) ---
  ...
  function playEKGBeep(rhythm) {
    ...
    // Frecuencia según ritmo (sonido característico)
    let frequency = 800; // base
    if (rhythm.includes('brady')) frequency = 600;      // más grave
    else if (rhythm.includes('tach') || rhythm.includes('svt')) frequency = 1000; // más agudo
    else if (rhythm.includes('fib') || rhythm.includes('flutter')) frequency = 500; // caótico
    else if (rhythm.includes('block')) frequency = 720;
    else if (rhythm.includes('asystole')) frequency = 400;
    else if (rhythm.includes('torsades')) frequency = 880;

    osc.type = 'square'; // beep más mecánico
    osc.frequency.value = frequency;

    // Envolvente rápida (ataque suave, decaimiento natural)
  ...
  NCLEX.registerTopic({
    ...
    // --- 2. GUÍA VISUAL EKG (MODIFICADA: verde, hover, sonido, mute) ---
    renderEKGStripGuide() {
      const hoverStyle = `
        <style>
          .ekg-strip {
